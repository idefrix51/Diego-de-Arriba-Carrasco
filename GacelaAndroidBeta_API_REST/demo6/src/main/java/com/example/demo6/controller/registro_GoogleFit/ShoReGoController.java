...
@RequestMapping("/api")
@CrossOrigin("*")
public class ShoReGoController {
    private DAOFactory factory = DAOFactory.getDAOFactory(DAOFactory.ORM);
    private Registro_GoogleFitDAO pacienteDAO = factory.getRegistro_GoogleFitDAO();

    @Operation(summary = "Endpoint para listar los registros de GoogleFit")
    @ApiResponses(value = {
    ...
    @GetMapping(value = "/registroGoogle", produces = {MediaType.APPLICATION_JSON_VALUE})
    public List<Registro_GoogleFitDTO> listarRegistroGoogleFit() {

        pacienteDAO.abrirConexion();
        List<Registro_GoogleFit> lista = pacienteDAO.listar();
        pacienteDAO.cerrarConexion();

        List<Registro_GoogleFitDTO> listaDTO = new ArrayList<>();
        for (Registro_GoogleFit paciente : lista) {
